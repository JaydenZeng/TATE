...
class MM:
    def __init__(self, is_training):
        ...
        wei_v = tf.reshape(tf.tile(tf.expand_dims(space_wei[0], 1), [1, self.config.max_visual_len*self.att_dim]), [self.config.batch_size, -1, self.att_dim])
        wei_a = tf.reshape(tf.tile(tf.expand_dims(space_wei[1], 1), [1, self.config.max_audio_len*self.att_dim]), [self.config.batch_size, -1, self.att_dim])
        wei_t = tf.reshape(tf.tile(tf.expand_dims(space_wei[2], 1), [1, self.config.max_text_len*self.att_dim]), [self.config.batch_size, -1, self.att_dim])


        enc_all = tf.concat([tf.multiply(common_v, wei_v), tf.multiply(common_a, wei_a), tf.multiply(common_t, wei_t)], 1)
